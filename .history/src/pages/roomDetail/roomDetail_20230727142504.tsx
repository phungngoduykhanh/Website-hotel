...
const RoomDetail: React.FC = () => {
  ...
  const openLightbox = (index) => {
    ...
    return (
      ...
      {isOpen && (
        <Lightbox
          mainSrc={document[photoIndex].linkimg} // Thay thế 'data.linkimg[photoIndex]' bằng 'document[photoIndex].linkimg'
          nextSrc={document[(photoIndex + 1) % document.length].linkimg} // Thay thế 'data.linkimg[(photoIndex + 1) % images.length]' bằng 'document[(photoIndex + 1) % document.length].linkimg'
          prevSrc={document[(photoIndex + document.length - 1) % document.length].linkimg} // Thay thế 'data.linkimg[(photoIndex + data.linkimg.length - 1) % data.linkimg.length]' bằng 'document[(photoIndex + document.length - 1) % document.length].linkimg'
