...
export default function RoomDetail() {
    ...
    // Giả sử bạn đã có một state hoặc hàm xử lý để lưu trữ thông tin phòng được chọn
    const [selectedRoomId, setSelectedRoomId] = useState(null);

    // Hàm xử lý khi chọn phòng, truyền room ID vào để xác định phòng được chọn
    const handleRoomClick = (roomId: PrismicDocument<Record<string, any>, string, string> | React.SetStateAction<null>) => {
            ...
            <Container className='container-fluid '>
                <Row data-aos="fade-up" className='roomdetail'>
                    ...
                    <Col xs={auto} md={12} lg={6}>
                        {document3 && document3.map((roomData) => (
                            <div key={roomData.id} className='room-item'>
                                <h3 className='type-roomdetail'>{roomData.data.name_room}</h3>
                                {/* Hiển thị thông tin phòng, ví dụ size và số người */}
                                <p>
                                    <FontAwesomeIcon className='icons-roomdetail' icon={faBed} />:{roomData.data.size}
                                    <FontAwesomeIcon className='icons-roomdetail' icon={faPerson} />:{roomData.data.people}
                                </p>
                                {/* Button để chọn phòng */}
                                <button onClick={() => handleRoomClick(roomData)}>Xem chi tiết</button>
                            </div>
                        ))}
                    ...
                    <Col data-aos="zoom-in-down" data-aos-duration="1000" xs={auto} md={auto} lg={5} className='justify-content-center'>
                        {/* Hiển thị nội dung chi tiết phòng được chọn */}
                        {selectedRoom && (
                            <div className='room-detail'>
                                <h3 className='type-roomdetail'>{selectedRoom.data.name_room}</h3>
                                <p><FontAwesomeIcon className='icons-roomdetail' icon={faBed} />:{selectedRoom.data.size}</p>
                                <p><FontAwesomeIcon className='icons-roomdetail' icon={faPerson} />:{selectedRoom.data.people}</p>
                                <p>{selectedRoom.data.content}</p>
                                {/* Hiển thị hình ảnh phòng */}
                                <PrismicImage field={selectedRoom.data.link_img} width={'80%'} height={'90%'} />
                            </div>
                        )}
                    </Col>
                </Row>
