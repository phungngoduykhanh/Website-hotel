    ...
    return (
        ...
        {document3.map((data, item, index) => (
          <Col key={index} data-aos="fade-up" data-aos-duration="500" xs={12} sm={6} md={6} lg={4}>
            <div className="image-wrapper-roomdetail d-flex justify-content-center">
              <img
                src={data.body[1].items[0].imglink} // Thay thế 'data.linkimg' bằng 'item.linkimg'
      ...
      {isOpen && (
        <Lightbox
          ...
          nextSrc={document3[(photoIndex + 1) % document3.length].linkimg} // Thay thế 'data.linkimg[(photoIndex + 1) % images.length]' bằng 'document[(photoIndex + 1) % document.length].linkimg'
          prevSrc={document3[(photoIndex + document3.length - 1) % document3.length].linkimg} // Thay thế 'data.linkimg[(photoIndex + data.linkimg.length - 1) % data.linkimg.length]' bằng 'document[(photoIndex + document.length - 1) % document.length].linkimg'
          onCloseRequest={() => setIsOpen(false)}
          onMovePrevRequest={() => setPhotoIndex((photoIndex + document3.length - 1) % document3.length)} // Thay thế 'data.linkimg.length' bằng 'document.length'
          onMoveNextRequest={() => setPhotoIndex((photoIndex + 1) % document3.length)} // Thay thế 'images.length' bằng 'document.length'
        />
      )}
                <Row>
