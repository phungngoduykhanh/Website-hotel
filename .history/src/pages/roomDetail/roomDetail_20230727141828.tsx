...
const RoomDetail: React.FC = () => {
    ...
    const [document, setIsOpen] = useState<boolean>(false);
    ...
    return (
        <>
            ...
            <Container className='container-fluid '>
                ...
                {document && (
                    <Lightbox
                        mainSrc={document.data.linkimg[photoIndex]}
                        nextSrc={document.data.linkimg[(photoIndex + 1) % images.length]}
                        prevSrc={document.data.linkimg[(photoIndex + document.data.linkimg.length - 1) % document.data.linkimg.length]}
                        onCloseRequest={() => setIsOpen(false)}
                        onMovePrevRequest={() => setPhotoIndex((photoIndex + document.data.linkimg.length - 1) % document.data.linkimg.length)}
                        onMoveNextRequest={() => setPhotoIndex((photoIndex + 1) % document.data.linkimg.length)}
                    />
                )} <br /><br />
                <Row>
