...
const RoomDetail: React.FC = () => {
    ...
    const imagesData = images.map((imageUrl: any) => ({
        data: {
          linkimg: imageUrl,
        },
      }));
    ...
    return (
        <>
            ...
            <Container className='container-fluid '>
                ...
                {imagesData.map((image: { data: { link_img: ImageFieldImage | null | undefined; }; }, index: React.Key | null | undefined) => {
                    return (
                        <Col key={index} data-aos="fade-up" data-aos-duration="500" xs={12} sm={6} md={6} lg={4}>
                            <div className="image-wrapper-roomdetail d-flex justify-content-center ">
                                <PrismicImage key={index} className="images-roomdetail" field={image.data.link_img}
                ...
                {isOpen && (
                    <Lightbox
                        mainSrc={imagesData[photoIndex]}
                        nextSrc={imagesData[(photoIndex + 1) % imagesData.length]}
                        prevSrc={imagesData[(photoIndex + imagesData.length - 1) % imagesData.length]}
