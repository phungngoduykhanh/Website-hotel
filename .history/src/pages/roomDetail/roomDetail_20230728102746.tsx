...
{document3 && document3.map((roomData) => (
  <div key={roomData.id} className='room-item'>
    ...
    <button onClick={() => handleRoomClick(roomData.id)}>Xem chi tiết</button>
  </div>
))}

{/* Hiển thị nội dung chi tiết phòng được chọn */}
{document3 && selectedRoomId && document3.map((roomData) => {
  if (roomData.id === selectedRoomId) {
    return (
      <div key={roomData.id} className='room-detail'>
        <h3 className='type-roomdetail'>{roomData.data.name_room}</h3>
        <p>{roomData.data.content}</p>
        {/* Hiển thị hình ảnh phòng */}
        <PrismicImage field={roomData.data.link_img} width={'80%'} height={'90%'} />
      </div>
    );
  }
  return null;
