...
export default function RoomDetail() {
  ...
  const handleRoomClick = (roomImages: React.SetStateAction<never[]>) => {
    ...
    return (
        <>
            ...
            <Container className='container-fluid '>
                ...
                {document3 && document3.length > 0 ? (
          document3.map((doc, index) => (
            <Col key={doc.id} data-aos="fade-up" data-aos-duration="500" xs={12} sm={6} md={6} lg={4}>
              <div
                className="image-wrapper-roomdetail d-flex justify-content-center"
                onClick={() => handleRoomClick(doc.data.body[1].items.map((item: { linkimg: any; }) => item.linkimg))}
              >
                <img
                  src={doc.data.body[1].items[0].linkimg} // Assuming there's at least one image in each room's "items" array
                  className="images-roomdetail"
                  alt={`Room ${index} Thumbnail`}
                />
                <div className="overlay-roomdetail d-flex justify-content-center"></div>
              </div>
            </Col>
          ))
        ) : (
          <p>No hotel rooms data available.</p>
        )}
                </Row>
      {/* Your existing code for Lightbox */}
      {isOpen && (
        <Lightbox
          mainSrc={currentRoomImages[photoIndex]} // Use the images for the currently selected room in the Lightbox
          nextSrc={currentRoomImages[(photoIndex + 1) % currentRoomImages.length]}
          prevSrc={currentRoomImages[(photoIndex + currentRoomImages.length - 1) % currentRoomImages.length]}
          onCloseRequest={() => setIsOpen(false)}
          onMovePrevRequest={() => setPhotoIndex((photoIndex + currentRoomImages.length - 1) % currentRoomImages.length)}
          onMoveNextRequest={() => setPhotoIndex((photoIndex + 1) % currentRoomImages.length)}
        />
      )}
 <br /><br />
                <Row>
