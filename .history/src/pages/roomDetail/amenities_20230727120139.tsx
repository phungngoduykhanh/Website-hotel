        ...
        <>
            ...
            <Container className='container-roomdetail'>
                ...
                <Row className='amenities'>
                    ...
                    {document && (
  <ul>
    {document.data.body[0].items[0].room_amenities.map((amenity, index) => (
      <li key={index}>
        <PrismicRichText field={amenity} />
      </li>
    ))}
  </ul>
)}
Trong ví dụ trên, chúng ta sử dụng điều kiện {document && ...} để kiểm tra xem biến "document" có tồn tại không. Nếu "document" không tồn tại (tức là null hoặc undefined), toàn bộ đoạn mã bên trong sẽ không được thực thi và tránh gây ra lỗi.

Nếu bạn vẫn gặp lỗi, hãy đảm bảo rằng biến "document" đã được định nghĩa và gán giá trị trước khi đoạn mã trên được thực thi.
