...
export default function RoomDetail() {
    ...
    const imageUrls = document3?.map((doc) => doc.data.body[0].linkimage) || [];
    // if (!document) {
  ...
  {document3 && document3.map((gallery, index) => (
    <Col key={index} data-aos="fade-up" data-aos-duration="500" xs={12} sm={6} md={6} lg={4}>
      <div className="image-wrapper-roomdetail d-flex justify-content-center">
        {gallery.data.body[1].item?.linkimg?.url ? (
          <img
            src={gallery.data.body[1].item.linkimg.url}
            onClick={() => openLightbox(index)}
            className="images-roomdetail"
            // alt={`Image ${imageUrls.data.body[0].linkimage}`}
          />
        ) }
