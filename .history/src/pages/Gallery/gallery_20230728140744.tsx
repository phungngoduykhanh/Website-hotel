...
import { usePrismicDocumentByUID, PrismicRichText, PrismicImage, useAllPrismicDocumentsByType, useSinglePrismicDocument } from '@prismicio/react'

const Gallery: React.FC = () => {
    useEffect(() => {
        AOS.init();
    }, []);
    const [Image] = useSinglePrismicDocument('gallery')
                ...
                {Image && Image.map((Image, index) => (
                        <Col key={index} data-aos="fade-up" data-aos-duration="500" xs={12} sm={6} md={6} lg={4}>
                            <div className="image-wrapper-roomdetail d-flex justify-content-center">
                                <img
                                    src={Image.data?.body?.item?.link_image.url}
