...
const Gallery: React.FC = () => {
    ...
    const getColSize = (): string => {
        ...
        // Xác định cấu hình cột dựa trên kích thước màn hình
        if (screenWidth >= 992) {
          // Kích thước lg (lớn hơn hoặc bằng 992px): 3 ảnh trên 1 hàng
          return '4';
        } else if (screenWidth >= 768) {
          // Kích thước md (lớn hơn hoặc bằng 768px): 2 ảnh trên 1 hàng
          return '6';
        } else {
          // Kích thước sm (nhỏ hơn 768px): 1 ảnh trên 1 hàng
          return '12';
        }
      };
    return (
  ...
  {Image &&
    Image.map((image, index) => (
      <Col key={index} data-aos="fade-up" data-aos-duration="500" xs={12} sm={6} md={4} lg={4}>
        <div className="image-wrapper-roomdetail ">
          {image.data &&
            image.data.body &&
            image.data.body[0].items &&
            image.data.body[0].items.map((item: any, itemIndex: number) => (
              <Col key={itemIndex} xs={12} sm={6} md={4} lg={4}>
                <img
                  key={itemIndex}
                  src={item.link_image.url}
                  onClick={() => openLightbox(index)}
                  className="images-roomdetail"
                  alt={`Image ${index + 1}, Item ${itemIndex + 1}`}
                />
              </Col>
            ))}
