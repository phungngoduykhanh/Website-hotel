...
const Gallery: React.FC = () => {
  ...
  const [documents] = useAllPrismicDocumentsByType('gallery');
    ...
    // ];
    // const images: string[] = documents?.map((document) => {
    //     return document.data.body[0].items[1].link_image?.url;
    //   }) || [];

    const images: string[] = documents?.flatMap((document) => {
        // Replace 'items' with the actual key that holds the array of items in your Prismic document
        return document.data.body[0].items.map((item) => {
