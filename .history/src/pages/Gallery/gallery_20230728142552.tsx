...
const Gallery: React.FC = () => {
    ...
    return (
      ...
      {Image && Image.map((image, index) => (
        <Col key={index} data-aos="fade-up" data-aos-duration="500" className="custom-col">
          <div className="image-wrapper-roomdetail d-flex justify-content-center">
            {image.data && image.data.body && image.data.body[0].items && image.data.body[0].items.map((item: { link_image: { url: string | undefined; }; }, itemIndex: React.Key | null | undefined) => (
              <img
                key={itemIndex}
