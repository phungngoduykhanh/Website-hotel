...
const Gallery: React.FC = () => {
  ...
  {Image && Array.isArray(Image) && Image.length > 0 && Image.map((image, index) => (
    <Col key={index} data-aos="fade-up" data-aos-duration="500" xs={12} sm={6} md={6} lg={4}>
      <div className="image-wrapper-roomdetail d-flex justify-content-center">
        {image.data && image.data.body && Array.isArray(image.data.body) && image.data.body.map((bodyItem, bodyIndex) => (
          bodyItem.items && Array.isArray(bodyItem.items) && bodyItem.items.map((item: { link_image: { url: string | undefined; }; }, itemIndex: number) => (
            <img
              key={`${index}-${bodyIndex}-${itemIndex}`}
              src={item.link_image?.url}
              onClick={() => openLightbox(index)}
              className="images-roomdetail"
              alt={`Image ${index + 1}, Item ${bodyIndex + 1}-${itemIndex + 1}`}
            />
          ))
        ))}
        <div className="overlay-roomdetail" onClick={() => openLightbox(index)}></div>
                ...
                {isOpen && (
                    <Lightbox
                        ...
                        nextSrc={Image[(photoIndex + 1) % images.length]}
                        prevSrc={Image[(photoIndex + images.length - 1) % images.length]}
