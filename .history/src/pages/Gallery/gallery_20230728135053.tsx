...
const Gallery: React.FC = () => {
    ...
    const [imageList, setImageList] = useState<PrismicDocument<Record<string, any>, string, string>[]>([]);

    // Assuming you have imported the 'useAllPrismicDocumentsByType' function properly
    const galleryData = useAllPrismicDocumentsByType('gallery');
  
    // Check if the data is loading or has been fetched successfully
    if (galleryData.state === 'loading') {
    ...
    // Update the state 'imageList' with the fetched data when 'galleryData' contains data
    useEffect(() => {
      if (galleryData.data) {
        setImageList(galleryData.data);
      }
    }, [galleryData.data]);
    return (
        <>
      ...
      {Image && Image.map((image, index) => (
        <Col key={index} data-aos="fade-up" data-aos-duration="500" xs={12} sm={6} md={6} lg={4}>
          <div className="image-wrapper-roomdetail d-flex justify-content-center">
            <img
              src={image?.data?.body?.item?.linkimage?.url} // Make sure to use optional chaining (?.) to avoid errors if any field is missing
