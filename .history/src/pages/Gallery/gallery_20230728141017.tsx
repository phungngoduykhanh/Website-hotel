  ...
  {Image && Image.map((image, index) => (
    <Col key={index} data-aos="fade-up" data-aos-duration="500" xs={12} sm={6} md={6} lg={4}>
      <div className="image-wrapper-roomdetail d-flex justify-content-center">
        {image.data.body && image.data.body.map((bodyItem: { items: any[]; }, bodyIndex: number) => (
          bodyItem.items && bodyItem.items.map((item: { url: string | undefined; }, itemIndex: number) => (
            <img
              key={`${bodyIndex}-${itemIndex}`}
              src={item.url}
