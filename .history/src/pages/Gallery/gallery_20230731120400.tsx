...
// Định nghĩa kiểu dữ liệu cho dữ liệu đường dẫn hình ảnh
interface ImageData {
    link_image: {
      url: string;
    };
  ...
  // Hàm để trích xuất các URL hình ảnh từ dữ liệu đã cho và tạo mảng mới
  function extractImageUrls(data: ImageData[]): string[] {
    return data.map(item => item.link_image.url);
  }
  
  // Dữ liệu dạng đường dẫn hình ảnh từ API
  const imageData: ImageData[] = [
    ...
    // Thêm các đối tượng hình ảnh khác vào đây...
  ];
  
  // Trích xuất URL hình ảnh và tạo mảng mới
  const imageUrls: string[] = extractImageUrls(imageData);
  
  // In mảng URL hình ảnh ra để kiểm tra
