	...
	const BookingForm: React.FC<BookingFormProps> = ({ onSubmit }) => {
		...
		const handleFormSubmit = (event: React.FormEvent) => {
			...
			if (formData.checkin_date === "") {
				...
				const totalPeople = (adults: any, children: any): number => {
					return parseInt(adults) + parseInt(children);
				}
				const peopleCount = totalPeople(formData.adults, formData.children);
				
				setTimeout(() => {
					navigate('/booking', { state: { checkin_date: formData.checkin_date, checkout_date: formData.checkout_date, people: peopleCount } })
				}, 1000);
			}
		};
